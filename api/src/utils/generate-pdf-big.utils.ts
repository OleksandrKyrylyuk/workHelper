...
export interface BadPhrase {
    phrase: string;
    why_bad: string;
    possible_client_reaction?: string;
...
export interface BetterReplyExample {
    ...
    manager_reply_problem: string;
}

export interface StrengthItem {
    strength: string;
    why_it_matters: string;
    example_from_conversation: string;
}
...
function renderStrengths(items?: StrengthItem[]): string {
    if (!items || items.length === 0) {
        return "<p>Недостатньо даних</p>";
    }

    return items.map(item => `
        <div class="card">
            <p><strong>Сильна сторона:</strong> ${escapeHtml(item.strength)}</p>
            <p><strong>Чому це важливо:</strong> ${escapeHtml(item.why_it_matters)}</p>
            <p><strong>Приклад із розмови:</strong> ${escapeHtml(item.example_from_conversation)}</p>
        </div>
    `).join("");
}

function renderMistakes(items?: MistakeItem[]): string {
    if (!items || items.length === 0) {
        return "<p>Недостатньо даних</p>";
    }
    ...
    return items.map(item => `
        <div class="card warning">
            <p><strong>Помилка:</strong> ${escapeHtml(item.mistake)}</p>
            <p><strong>Вплив:</strong> ${escapeHtml(item.impact)}</p>
            <p><strong>Як виправити:</strong> ${escapeHtml(item.how_to_fix)}</p>
        </div>
    `).join("");
}

function renderGoodPhrases(items?: GoodPhrase[]): string {
    if (!items || items.length === 0) {
        return "<p>Недостатньо даних</p>";
    }
    ...
    return items.map(item => `
        <div class="card">
            ...
            <p><strong>Чому вдала:</strong> ${escapeHtml(item.why_good)}</p>
            <p><strong>Ефект для клієнта:</strong> ${escapeHtml(item.effect_on_client)}</p>
        </div>
    `).join("");
}

function renderRecommendations(items?: Recommendation[]): string {
    if (!items || items.length === 0) {
        return "<p>Недостатньо даних</p>";
    }
    ...
    return items.map(item => `
        <div class="card">
            <p><strong>Рекомендація:</strong> ${escapeHtmlWithBreaks(item.recommendation)}</p>
            <p><strong>Як застосувати:</strong> ${escapeHtmlWithBreaks(item.how_to_apply)}</p>
            <p><strong>Приклад фрази:</strong><br />${escapeHtmlWithBreaks(item.example_phrase)}</p>
        </div>
    `).join("");
}
...
function generateCallAnalysisHtml(data: CallAnalysisJson) {
  ...
  `).join("") || "<p>Недостатньо даних</p>";
    ...
    <div class="card warning">
      ...
      <p><strong>Можлива реакція клієнта:</strong> ${escapeHtml(item.possible_client_reaction || "Недостатньо даних")}</p>
...
`).join("") || "<p>Недостатньо даних</p>";
    ...
    <div class="card">
      <p><strong>Ситуація:</strong> ${escapeHtmlWithBreaks(item.situation)}</p>
      <p><strong>Проблема відповіді менеджера:</strong> ${escapeHtmlWithBreaks(item.manager_reply_problem)}</p>
      <p><strong>Краща відповідь менеджера:</strong><br />${escapeHtmlWithBreaks(item.better_manager_reply)}</p>
    </div>
  `).join("") || "<p>Недостатньо даних</p>";
...
<body>
  <div class="page">
    ...
    <div class="section">
      ...
      ${renderGoodPhrases(data.good_phrases)}
    </div>
...
export async function generateCallAnalysisPdfBig(analysisJson: CallAnalysisJson): Promise<Buffer> {
